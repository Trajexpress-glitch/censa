...
function PostCard({ post, t, onOpen, compact }) {
  ...
  const toggleComments = () => {
    if (post.commentsLocked) return;
    if (compact) { onOpen && onOpen(post); return; }
    setShowComments(v => !v);
  };
      ...
      <div style={{ display: 'flex', gap: 13 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, marginLeft: -8 }} onClick={(e) => e.stopPropagation()}>
            ...
            <button className={"act" + (showComments ? ' on-rep' : '')} onClick={toggleComments} disabled={post.commentsLocked}
              style={{ opacity: post.commentsLocked ? 0.4 : 1 }}>
              <Icon name="comment" size={17} /> {post.commentsLocked ? '—' : fmt(commentCount)}
            </button>
          ...
          {showComments && !compact && !post.commentsLocked && (
            <InlineComments post={post} t={t} onCount={setCommentCount} />
          )}
        </div>
      </div>
    </article>
...
/* ---- Réponses en ligne (dans le flux) : chargées + persistées (Supabase) ---- */
function InlineCommentRow({ c }) {
  const author = uget(c.author);
  return (
    <div style={{ display: 'flex', gap: 10, padding: '9px 0' }}>
      <Avatar user={author} size={34} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ background: 'var(--surface-2)', borderRadius: 14, padding: '8px 13px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <span style={{ fontWeight: 700, fontSize: 13.5 }}>{author.name}</span><Badge user={author} />
            <span style={{ color: 'var(--text-faint)', fontSize: 12 }}>@{author.handle}</span>
          </div>
          <p style={{ marginTop: 2, fontSize: 14, lineHeight: 1.45, color: c.alarm ? 'var(--alarm)' : 'var(--text)',
            whiteSpace: 'pre-wrap', textWrap: 'pretty' }}>{L(c.text)}</p>
        </div>
        <div className="mono" style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 4, marginLeft: 13 }}>{L(c.time)}</div>
      </div>
    </div>
  );
}

function InlineComments({ post, t, onCount }) {
  const me = uget('me');
  const cloud = window.CENSA_CLOUD;
  ...
  const load = useCallback(async () => {
    if (cloud && cloud.ready()) {
      try { const c = await cloud.loadComments(post.id); setList(c); if (onCount) onCount(c.length); }
      catch (e) { setList([]); }
    } else { setList([]); }
  }, [post.id]);

  useEffect(() => { load(); }, [load]);
  // mise à jour en direct : une réponse d'un ami arrive via Realtime
  useEffect(() => {
    const h = (e) => { if (e.detail && e.detail.postId === post.id) load(); };
    window.addEventListener('censa:comment-new', h);
    return () => window.removeEventListener('censa:comment-new', h);
  }, [post.id, load]);
  ...
  const send = async () => {
    ...
    setBusy(true);
    let added = null;
    if (cloud && cloud.ready()) { try { added = await cloud.addComment(post.id, text); } catch (e) {} }
    if (!added) added = { id: 'local_' + Date.now(), author: 'me', text: { fr: text, en: text }, time: t.now };
    setList(l => { const next = [...(l || []), added]; if (onCount) onCount(next.length); return next; });
    setReply('');
    setBusy(false);
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <textarea ref={ref} value={reply} rows={1} placeholder={t.reply_ph}
            onChange={(e) => { setReply(e.target.value); e.target.style.height = 'auto'; e.target.style.height = Math.min(e.target.scrollHeight, 160) + 'px'; }}
            onKeyDown={(e) => { if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) { e.preventDefault(); send(); } }}
            style={{ width: '100%', resize: 'none', border: '1px solid var(--border-br)', borderRadius: 12,
              background: 'var(--surface-2)', color: 'var(--text)', fontSize: 14.5, lineHeight: 1.5, padding: '9px 12px', fontFamily: 'var(--font-body)' }} />
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
            <button className="btn btn-primary" onClick={send} disabled={!reply.trim() || busy}
              style={{ opacity: (reply.trim() && !busy) ? 1 : 0.5, padding: '7px 16px', fontSize: 13 }}>{t.reply}</button>
          </div>
        </div>
      </div>
      <div style={{ marginTop: 4 }}>
        {list === null ? (
          <div className="mono" style={{ padding: '12px 4px', fontSize: 12.5, color: 'var(--text-faint)' }}>…</div>
        ) : list.length === 0 ? (
          <div className="mono" style={{ padding: '10px 4px', fontSize: 12.5, color: 'var(--text-faint)' }}>
            {L({ fr: 'Soyez le premier à répondre.', en: 'Be the first to reply.' })}
          </div>
        ) : list.map(c => <InlineCommentRow key={c.id} c={c} />)}
      </div>
    </div>
  );
}
...
Object.assign(window, { Composer, PostCard, PostVideo, Feed, InlineComments, InlineCommentRow });
