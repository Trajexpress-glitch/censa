...
function App() {
  ...
  async function refreshFeed() {
    ...
    try { const v = await window.CENSA_CLOUD.loadVideos(); if (Array.isArray(v)) setVideos(v); } catch (e) {}
    loadMembers();
  }
  ...
  useEffect(() => {
    ...
    window.addEventListener('censa:video-new', soon);
    window.addEventListener('censa:comment-new', soon);
    window.addEventListener('censa:reaction-new', soon);
    return () => { clearTimeout(timer); window.removeEventListener('censa:feed-new', soon); window.removeEventListener('censa:story-new', soon); window.removeEventListener('censa:video-new', soon); window.removeEventListener('censa:comment-new', soon); window.removeEventListener('censa:reaction-new', soon); };
  }, [authed]);
  ...
  const addVideo = async (video) => {
    if (window.CENSA_CLOUD && window.CENSA_CLOUD.ready()) {
      const nv = await window.CENSA_CLOUD.createVideo(video);
      if (nv) { setVideos(v => [nv, ...v]); return; }
    }
    setVideos(v => { const next = [video, ...v]; try { localStorage.setItem('censa_videos', JSON.stringify(next)); } catch (e) {} return next; });
  };
